...
moderationRouter.get('/players/search', (req: AuthenticatedRequest, res: Response) => {
  ...
  return res.json({
    ...
    tracked: playerTracker.list().filter((p) =>
      p.gamertag.toLowerCase().includes(q.toLowerCase()) || p.xuid.includes(q)
    )
  });
});
...
moderationRouter.post('/players/resolve', requireRole(UserRole.MODERATOR), async (req: AuthenticatedRequest, res: Response) => {
...
moderationRouter.post('/players/join', requireRole(UserRole.MODERATOR), (req: AuthenticatedRequest, res: Response) => {
  ...
  const player = playerTracker.recordJoin({
    gamertag,
    xuid,
...
moderationRouter.post('/:id/deactivate', requireRole(UserRole.MODERATOR), (req: AuthenticatedRequest, res: Response) => {
...
moderationRouter.post('/gdpr/anonymize', requireRole(UserRole.ADMIN), (req: AuthenticatedRequest, res: Response) => {
...
moderationRouter.post('/allowlist/sync', requireRole(UserRole.ADMIN), async (req: AuthenticatedRequest, res: Response) => {
  ...
  // Prefer live agent tunnel when connected; otherwise return the write plan for the operator/agent.
  if (server.agentId && agentGateway.isNodeConnected(server.agentId)) {
    try {
      const result = await agentGateway.sendCommand(server.agentId, server.id, 'ALLOWLIST_SYNC', {
        entries,
      ...
      // Best-effort RCON reload via HostProvider
      const provider = HostProviderFactory.getProvider(server.hostProvider || HostProviderType.DOCKER_AGENT);
      await provider.executeRcon(server, plan.reloadCommand);

      return res.json({ success: true, plan, agentResult: result });
    } catch (err) {
