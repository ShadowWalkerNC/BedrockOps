...
- **`packages/bds-bots`**: Offline Bedrock fake-player harness (`bedrock-protocol`) for local live-BDS integration tests.
...
- **Live BDS + fake players (Linux x86_64):** prefer `./scripts/bds/download-bds.sh --bot-compat` (pins a BDS build that matches current `bedrock-protocol`), then `./scripts/bds/configure-bds.sh` + `./scripts/bds/ensure-raknet-native.sh`. Run BDS with `./scripts/bds/run-bds.sh` or the full stack via `./scripts/start-local-bds.sh`. Drive joins with `pnpm --filter @mc-admin/bds-bots bot:<join|chat|flood|churn|ping>`. Details in `docs/local-bds-testing.md`. Mojang CDN needs a non-default User-Agent (handled by the script). Bots require `online-mode=false`.
...
- **Stale `.next` / missing `vendor-chunks`:** if Next throws `Cannot find module './chunks/vendor-chunks/next@…'`, stop the web dev server and run `pnpm --filter @mc-admin/web clean` then `pnpm --filter @mc-admin/web dev` (or `pnpm --filter @mc-admin/web dev:clean`). Cloning under OneDrive/iCloud sync folders makes this much more common — prefer a non-synced path.
