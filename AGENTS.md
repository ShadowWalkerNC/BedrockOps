...
- **Live BDS + fake players (Linux x86_64):** one-shot `./scripts/bds/run-bot-e2e.sh` (add `--with-api` to assert player ingest + `JOIN_FLOOD_DETECTED`). Or manually: `./scripts/bds/download-bds.sh --bot-compat` → `configure-bds` → `ensure-raknet-native` → `run-bds` / `start-local-bds`. Details in `docs/local-bds-testing.md`. Mojang CDN needs a non-default User-Agent (handled by the script). Bots require `online-mode=false`. Offline empty XUIDs are synthesized on ingest.
